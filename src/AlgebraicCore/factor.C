...
//???      return (IsInvertible(facs.myRemainingFactor()) && NumFacs == 1 && mult == 1); // see  [redmine 1710]
  ...
  RingElem RadicalOfPoly(ConstRefRingElem f)
  {
    ...
    if (!IsZZ(k) && !IsField(k)) CoCoA_THROW_ERROR(ERR::NotField, "RadicalOfPoly");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/factor.C,v 1.35 2023/03/09 22:41:07 abbott Exp $
// $Log: factor.C,v $
