...
//???   std::lock_guard<std::mutex> lock(myMutex);
    if (IsZero(myNegatveSum)) return myPositiveSum;
    if (IsZero(myPositiveSum)) return myNegatveSum;
    myPositiveSum += myNegatveSum; myNegatveSum = 0;
    if (sign(myPositiveSum) >= 0) return myPositiveSum;
    swap(myPositiveSum, myNegatveSum);
    return myNegatveSum;
  }
  ...
  {
    out << "SumBigInt(...)";
//???    out << "SumBigInt(myNegativeSum=" << FloatStr(S.myNegativeSum) << ", myPositiveSum=" << FloatStr(S.myPositiveSum << ")";
    return out;
  }
...
//FASTER???      if (n <= 0xf) return topbittab[n];
      unsigned char pos = 0;
      if (n > 0xffffffff) { n >>= 32; pos += 32; } // optimizes out on 32-bitters (hopefully)
  ...
  {
    if (IsZero(n))
      CoCoA_THROW_ERROR(ERR::LogZero, "FloorLog2");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/BigIntOps.C,v 1.19 2023/03/09 22:34:18 abbott Exp $
// $Log: BigIntOps.C,v $
