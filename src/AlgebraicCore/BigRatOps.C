...
//???    std::lock_guard<std::mutex> lock(myMutex); // cannot use auto :-(
    ...
    if (IsZero(sum)) return BigRat(myBigIntPart.myTotal());

    const std::size_t i = ourIndex(sum);
    myGeobucket[i] = sum;  //??? resize myGeobucket ???
  ...
  {
    out << "SumBigRat(...)";
///???    out << "SumBigRat(myBigIntPart=..., myGeobucket=...)";
    return out;
  }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/BigRatOps.C,v 1.17 2023/03/09 22:34:18 abbott Exp $
// $Log: BigRatOps.C,v $
