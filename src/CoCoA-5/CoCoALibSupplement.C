...
//???    const ErrorInfo ErrMesg(ERR::ArgTooBig, "FactorINT_TimeOut_forC5");
    ...
    if (secs < 0 || secs > 10000) CoCoA_THROW_ERROR("Time limit mustbe between 0 and 10000", "FactorINT_TimeOut_forC5");
...
//   }
  ...
  {
    long n;
    if (!IsConvertible(n, N)) CoCoA_THROW_ERROR(ERR::ArgTooBig, "GraeffeN");
    return GraeffeN(f, n);
  }
  ...
  {
    long n;
    if (N < 1) CoCoA_THROW_ERROR("arg1: n must be >= 1", "cyclotomic");
    if (!IsConvertible(n, N)) CoCoA_THROW_ERROR(ERR::ArgTooBig, "cyclotomic (arg1)");
...
// ALMOST    return VectorLong(LRSDegeneracyOrders(f));
  ...
  {
    unsigned long n;
    if (!IsConvertible(n, N)) CoCoA_THROW_ERROR(ERR::ArgTooBig, "IsLRSDegenerateOrder");
